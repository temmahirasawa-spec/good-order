...
export function CartButton({
  ...
  return (
    <button
      ...
      onClick={onClick}
      aria-label="カートを見る"
      className={`flex gap-[var(--space-8)] h-[var(--size-control-lg)] items-center rounded-full bg-surface-white border border-border pl-[18px] pr-[20px] ${className}`}
      style={{ boxShadow: "var(--shadow-card)" }}
    >
...
export function LinkButton({
  ...
  const base = `flex gap-[var(--space-4)] items-center justify-center p-[var(--space-16)] rounded-xs border border-text-secondary ${className}`;
  const content = (
    <>
