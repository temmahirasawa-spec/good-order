...
**2026-09-13 に案Aへ変更。** それまでは「売り切れチップ」＋「公開トグル」の2つが別々の見た目で並んでいて、
トグルが何のスイッチか分からなかった（天真の指摘）。**1列・1つの状態チップ**にまとめた。
3案の比較は `.claude/verification/2026-09-13-admin-state/`。

- 一覧の行: 価格の右に **状態チップ1つ**（PC / SP 共通）。状態は文字で出す
  - **販売中**（白・枠線）… 押すと売り切れ
  - **売り切れ**（status/urgent-subtle 地に status/urgent）… 押すと販売中
  - **非表示**（bg/tertiary 地に text/tertiary）… 押しても変わらない。行全体を薄くする
- 押した瞬間に切り替わり（楽観的更新）、失敗したときだけ戻る
- 売り切れのときはサムネにも「SOLD OUT」の帯（写真があるときだけ）
- **公開・非公開の一覧からの切り替えは廃止**し、編集パネルに集約した（営業中の操作ではないため）
- 編集パネル: 「公開する」の下にトグル **「売り切れにする」**。補足「注文画面に残したまま「SOLD OUT」と表示され、カートに入れられなくなります」
- 文言と判定は `lib/soldOut.ts`（`MENU_ITEM_STATE_LABEL` / `menuItemState()`）に1か所化
...
|---|---|---|---|---|---|
| **A 帯** | 写真の色を抜いて、真ん中に墨の帯「SOLD OUT」 | 白黒になり少し沈む | 押せないグレーのピル「SOLD OUT」 | 「＋」の代わりにピル（サムネがあれば小さい帯も） | 数量限定・日替わりが多い店。遠目でも一目で分かる。**YORKYS 向き（AI推奨）** |
...
### 2026-09-13: 「画像なしに見える」への対応（と、白黒にした件の取り消し）

天真の指摘「背景に画像が表示されていないと、画像なしになっちゃうように見えるから修正して」。

**原因は透け具合ではなく、写真が1枚も無い商品だった。** 灰色の空箱に黒帯だけが乗るので、
「画像の読み込みに失敗した」ようにしか見えない。対応:

- **写真があるときだけ帯を出す**（`MenuCard` / `MenuListRow` / `RecommendCard` / `CartItemRow` /
  `AdminMenuRow`。商品詳細は元から写真があるときだけ）。写真が無い商品の「売り切れ」は
  `SoldOutPill`（操作部のピル）だけで伝える
- 写真の見え方（`.sold-out-photo` = `opacity: .4`）は**元のまま**

> ⚠ この過程で AI が勝手に見え方を白黒（`grayscale`）に変えた（PR #70）。
> **指示されていない変更で、天真から「白黒にしろとかいったっけ？」と指摘されて取り消した。**
> 見え方（デザイン）の変更は CLAUDE.md 3章の「止まって確認する」に当たる。**直せと言われた点だけを直すこと。**

Figma も元の見え方（`Dim` = surface/white 60%、彩度はそのまま）に戻してある。
この往復で分かった Figma の注意点（次に触るとき用）:
- **ブレンドモード（SATURATION）で彩度を落とすと画像が真っ黒に潰れる。** 使わない
- **変数にバインドした塗りに `opacity` を載せると、インスタンス側で 1 に戻ることがある。**
  オーバーレイの濃さは**ノードの `opacity`** で持つこと（読み戻して確認する）
