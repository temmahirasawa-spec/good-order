...
### 2026-09-13 の修正: 写真の沈め方

最初は写真を `opacity: .4` で薄くしていたが、**白い皿・白い料理の写真が白飛びして「画像が無い」ように見えた**
（天真の指摘「背景に画像が表示されていないと、画像なしになっちゃうように見える」）。
透かすのをやめ、`.sold-out-photo`（`filter: grayscale(1) brightness(0.95)`、`app/globals.css`）で**色だけ抜いて少し沈める**形にした。
写真の形が残るので「写真はある。ただし今は買えない」と読め、カラーの通常カードと並ぶと差も一目で分かる。PR #70。

Figma 側も同じ見え方に揃えた:
- 写真の上の `Dim` は **墨 8%**（白 60% をやめた）。**塗りの opacity ではなくノードの opacity で持つ**
  — 変数にバインドした塗りに opacity を載せるとインスタンス側で 1 に戻る事象があったため（2026-09-13 に実際に踏んだ）
- 実写が入っているところは `ImagePaint.filters` の `saturation: -1` / `exposure: -0.04` で白黒にする
- **ブレンドモード（SATURATION）は使わない。** 試したところ画像が真っ黒に潰れた
