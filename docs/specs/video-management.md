...
コードは入っているが `supabase/store_media.sql` はまだ流していない。適用するまでは
`lib/storeMedia.ts` の `STORE_MEDIA_FALLBACK`（＝移行前にコードへ直接書かれていた
アセット）が使われ、お客様側の見え方は従来どおりになる。適用手順は G を参照。

_作成: 2026-08-03 ／ 実装反映: 2026-08-03_
...
**旧F-5：`stores` の anon SELECT — 既に開いている。**
...
**Supabase の契約プラン自体は未確認**（リポジトリからは判別できない）。無料枠のままなら、D-3 のビットレートをさらに下げるか、長さの上限を短くする必要がある。
...
| # | 項目 | 形式 | 備考 |
|---|---|---|---|
| 1 | トップページに動画を表示する | `components/ui/ToggleSwitch.tsx`（40×22） | OFF で動画枠ごと消える |
| 2 | 動画（現在の状態） | 80×80 のタイル＋再生マーク | `MediaUploaderField` と同じ見た目 |
| 3 | 削除 | タイル右上の× → 確認ダイアログ | C-6 |
| 4 | 追加 / 差し替え | 破線の全幅ボタン（h-40） | 動画の有無でラベルが変わる |
| 5 | 最終更新 | タイルの右に `最終更新: 2026/08/03 14:32` | 旧F-11 = 表示する |
| 6 | 注釈テキスト | 静的（店舗は編集できない） | スロットごとに文言が違う。C-4 |
...
**2か所で切り取られ方が違うので、同じ文言は使っていない。**文言は `app/admin/(protected)/settings/page.tsx` の `SLOT_CONFIG` にある。

**① 注文ホームのヒーロー動画**（`hint`: メニュー一覧の先頭に、横長の帯として出ます。）
...
**② 二次元コード着地画面の背景動画**（`hint`: お客様が二次元コードを読み取って最初に開く画面の、背景いっぱいに出ます。）
...
解消したもの（旧F-1 / F-4 / F-5 / F-8 / F-9 / F-10 / F-11）は削除した。
...
| **F-6** | **Supabase の契約プラン**（未確認） | Free / Pro | A-8 の概算では、無料枠だと約36客/日で転送上限に当たる。無料枠のままなら D-3 のビットレートを下げるか長さの上限を短くする必要がある |
...
6. 管理画面（manager でログイン）→ サイドナビ「店舗設定」→「トップページ」で、トグル・差し替え・削除が動くことを確認する
