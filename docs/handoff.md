...
- `<Link>` なので長押し・新規タブも効く。`aria-label="トップへ戻る"`
- タップ領域は `p-4` で 109×38 → 117×46 に（44px の下限を満たす）。見た目は変わらない
- 効くページ: トップ・カテゴリー一覧・Menu・テイクアウト・注文履歴・注文完了（`OrderHeader` を使う全ページ）

### 2026-09-19: ヘッダーの ☰ / × を一般的なハンバーガーメニューの挙動に

- 症状（天真の指摘）: ☰ → Menu ページ → 「パンケーキ」→ そのカテゴリー一覧で × を押すと Menu ページに戻り、
  「ハンバーガーメニューがまた開いた」ように見える
- 原因: 中身のページ（カテゴリー一覧・テイクアウト・注文履歴）も `OrderHeader variant="close"`（× ＋ `router.back()`）だった
- 直し: **× は Menu ページだけ**にし、中身のページは常に ☰（`variant` 既定の open）。
  ☰ → Menu → × で元のページに戻る、という一般的な動きになった
- 変えたページ: `app/order/[category]`、`app/order/takeout`、`app/history`。TOP と完了画面はもともと ☰
- 部品（`OrderHeader`）自体は変えていない。ページが渡す variant を変えただけ

### 2026-09-19: ハーフモーダルの × を位置固定に

- 天真の指示「すべてのハーフモーダルの右上の ✗ ボタンは位置固定」。全部を調べて、**ずれていたのは 2 つだけ**だった
  - `components/order/ItemDetailOverlay.tsx`（商品詳細）: × と左上の戻る矢印がスクロール領域の中にあった → シート直下に出して `absolute ... z-10` で固定
  - `components/StoreInfoModal.tsx`（店舗情報）: ヘッダー行がスクロール領域の中にあった → `sticky top-0 z-10 bg-surface-white` に。
    地の白で本文が透けない。ヘッダーに `pb-8` を足し、写真の `mt-16` を `mt-8` に下げて、開いた直後の余白は元のまま
- 既に固定されていたもの: 管理画面のメニュー編集・カテゴリ編集パネル、席設定、スタッフ呼び出しの設定（いずれもヘッダーが `overflow-y-auto` の外）
- スクロールしない小さなシート（スタッフ呼び出し・絞り込みのプレースホルダー）は対象外
- ⚠ `StaffCallSheet` は max-height もスクロールも持たない。呼び出しの項目を 8 つまで増やすと、小さい端末で下が切れる可能性がある（未対応）

### 2026-09-19: ドリンクの一覧ボタンを「詳細を見る」に
...
- 今後の検討: 全商品に「詳細を見る」と「カートに入れる」を併置するか、すべて「詳細を見る」に統一するか
...
- 2026-09-18 に第2部（当日の運用・7ページ）と第3部（設定・10ページ）も書いた。中身はコードと docs/opening-checklist・preopen-verify から起こした
  （Explore エージェントで画面の文言を洗い出し）。**管理画面のスクショは未撮影**: ローカルは本番 DB につながっていて管理者アカウントが無いため。
  天真が見本用アカウントを `.env.local`（`HELP_SHOT_EMAIL` / `HELP_SHOT_PASSWORD` を想定）に置いたら、ログインして撮る
- 書いていて見つけた実装側の気づき（未対応）: カテゴリ管理の「区分」のヒント文「フードが先・ドリンクが後に並ぶ」は今の動きと違う（並びは表示順のみ）／
  ダッシュボードの席数 40 固定・ドリンク判定がカテゴリ名固定／卓上カードの店名が YORKYS 固定／カート画面の写真なし商品はグレー枠が残る

### 2026-09-17: 一覧の「カートに入れる」はその場で入れる・写真なしの行は枠を出さない

- TOP・カテゴリ・テイクアウトの一覧で「カートに入れる」を押したら**その場でカートに入る**（天真の決定）。
  オプションのある商品も既定値（1つ選ぶ型は 1 番目、複数選ぶ型は無し）で入れる。詳細シートを経由しない。
  `docs/specs/menu-options.md` 3-2 を更新
- 一覧の行（`MenuListRow`）で写真が無い商品はグレーの枠ごと出さず、品名と価格を左に詰める（グレーの空箱が「準備中」に見えるため）
...
- 洋輔さんの要望で厨房伝票の補足を小さくした（2026-09-17）。品名は今のまま（font_a・縦2倍・太字）、
  「＋HOT」「先出し」「でき次第」は小さい書体 font_b・縦2倍（約0.7倍）、「食後」の黒帯はそのまま。
  3案（品名を1.5倍にする案など）を出したが、天真の決定は「品名は変えず補足だけ小さく」。`lib/receipt.ts`
...
# 2026-09-08: トップの構成・サブカテゴリー・文字メニュー・ブランドカラー（オリーブモス）

仕様: `docs/specs/home-layout.md` / `docs/specs/menu-text-rows.md` / `docs/specs/brand-color.md`（それぞれ末尾に実装の表）。
洋輔さん向けの共有: `docs/share/2026-09-08-home-layout-and-brand-color.html`。

## 何をしたか（1日で Figma → 実装まで）

1. **トップの見直し**: 各区画は「上位 N 件（既定5）＋ 見出し右の『すべてを見る』ボタン」。区画の並びは管理画面「カテゴリ管理」の並び順そのまま（同日昼の「ドリンクを最上部に固定」は夜に取り下げ。管理画面で動かせる方がよい）。
   2026-09-07 の PR #60（トップに全件）は、ユーザーのメンタルモデルに合わないと天真が判断して1日で取り下げ、この形に置き換えた。
2. **サブカテゴリー**: `categories.parent_id` の2階層。トップのドリンク区画にはチップ（すべて／カフェ／…）、一覧ページには下線タブ（押すと見出しへスクロール）。
   **チップ＝絞り込み、タブ＝移動** と役割を分けた（天真の指定）。
3. **文字メニュー**: 写真が無いカテゴリーは自動で「文字の行」（`MenuListRow`）。写真の無い商品詳細は KV を出さない。
4. **ブランドカラー**: オリーブモス #5E6B4A を既定に。Figma の YORKYS モードごと差し替え、黄色は「YORKYS Yellow（旧）」に退避。
   店舗が管理画面「表示設定 ＞ ブランドカラー」で変えられる（`stores.brand_accent`、サーバー側で CSS 変数を上書き）。

## ⚠ 流す順番: SQL 3本 → マージ

本番（`good-order` = oiropkuvaenebmlicrac）で先に流す:
`supabase/category_subcategories.sql` / `supabase/category_list_style.sql` / `supabase/store_brand_accent.sql`。
カテゴリーの読み取りだけは、列が無くても旧列で読み直す安全弁を入れてある（`lib/api.ts fetchCategories`）。
ブランドカラーも列が無ければ既定色のまま。管理画面のブランドカラー保存だけは SQL が無いと失敗する。

## 判断（天真未確認のもの。いつでも覆せる）

- トップの文字の行の2行目は、サブカテゴリーがある区画では**区分名**、無ければ商品の説明文。
- 一覧ページの上部は「ロゴのヘッダー＋カテゴリー名」（既存の一覧と同じ）。旧 Filter Bar（カスタマイズ／アレルギー）は置かない。
- 親に直接ぶら下がる商品は、一覧ページでサブカテゴリーの後に「その他」でまとめる。
- ブランドカラーの派生色（押した時 / 濃い / 薄い地 / 墨の上）は HSL で機械的に作る（`lib/brandColor.ts`）。管理画面のプレビューと本番が同じ式。
- `bg-warm` もモスに合わせて `#F3F3F0` にした（カート画面の地）。
- Figma の**ロゴの黄色は触っていない**（ブランド資産）。
...
## 2026-09-13: 売り切れの写真が「画像なし」に見えた件の修正

天真の指摘「SOLD OUT の画像表示が2枚とも見にくい」「背景に画像が表示されていないと、画像なしになっちゃうように見える」。

- **原因**: 売り切れの写真を `opacity: .4` で薄くしていたため、白い皿・白い料理の写真が白飛びしていた。
  帯だけが白地に浮き、「画像の読み込みに失敗した」ように見える。**本番の `/dev/ui` の実物で再現を確認した**
- **修正（PR #70、`ad4700f`）**: `app/globals.css` に `.sold-out-photo`（`filter: grayscale(1) brightness(0.95)`）を追加し、
  一覧のカード3種・文字の行・おすすめ・カート・商品詳細の KV・メニュー管理のサムネの6箇所で共用。
  透かすのをやめて色だけ抜くので、写真の形が残り、カラーの通常カードと並ぶと差も一目で分かる
- **Figma も同じ見え方に揃えた**（`docs/specs/sold-out-and-receipt-copies.md` 10章の追記）。踏んだ罠2つ:
  - **ブレンドモード `SATURATION` は使えない**（画像が真っ黒に潰れる）。`ImagePaint.filters.saturation = -1` を使う
  - **変数にバインドした塗りに `opacity` を載せると、インスタンス側で 1 に戻ることがある**。
    `Dim` は塗りではなく**ノードの `opacity`** で 8% にした（読み戻して確認済み）
- **洋輔さん向け資料を作り直した**: 画像を半分の列に押し込むのをやめ、**幅いっぱい・部品ごとに切り出し**て並べた。
  開発用の注釈（`/dev/ui` の説明文）が写り込まないよう、横幅は外枠・上下は中身から取って切り出している
  （スクリプトは scratchpad の `shoot2.mjs`。リポジトリには入れていない）
- `npm run check` / `npm run design:figma`（構造・パディング問題なし、新しい違反なし）とも通過

## 2026-09-13（続き）: 白黒の取り消し／カルーセルの進捗バー／管理画面の状態UI（3案・選択待ち）

### 1. 売り切れの写真を白黒にしたのは誤り。取り消した（PR #72）

天真「白黒表示にしろとかいったっけ？1枚目の添付の見え方でいいんだけど」。**指示されていない見え方の変更だった。**
`.sold-out-photo` は `opacity: .4`（元の見え方）に戻し、Figma も `Dim` = surface/white 60% に戻した。

本来の指摘「背景に画像が表示されていないと、画像なしになっちゃうように見える」の原因は
**写真が1枚も無い商品**。灰色の空箱に黒帯だけが乗っていた。→ **写真があるときだけ帯を出す**ようにした。
`/dev/ui` に「写真が無い商品」のカードを追加して、この状態も目視できるようにしてある。

> **教訓**: 指摘された点だけを直す。見え方（デザイン）を良かれと思って変えない（CLAUDE.md 3章）。

### 2. カルーセルのドット → 進捗バー（PR #73、本番反映済み）

天真の指摘「一番右まで行っても右側のドットが点灯せず、一番右まで行ききらない」。

- **原因**: 現在地を「スクロール量 ÷ カード1枚の幅」で出していた。スクロールの最大値は
  「枚数 × 1枚の幅」より小さい（最後の1枚は画面内に収まる）ので、**最後のドットには構造上たどり着けない**
- **対応**: `hooks/useScrollProgress.ts`（新規）でスクロール量そのものから比率を出す。
  `(スクロール位置 + 見えている幅) ÷ 中身の全幅` なので**右端でちょうど 1**。カードの枚数・幅・ガターに依存しない
- `components/ui/CarouselProgress.tsx`（新規）= 120×6・角丸3 のバー。`CarouselDots.tsx` は削除
- **ベストセラー（MenuCarouselWide）にも追加**。自動で往復する RecommendCarousel には付けない
- ⚠ hook は「スクロールできない」を **null** で返す。1 で返すと右端（＝1）と区別がつかず、
  **一番右まで寄せた瞬間にバーが消える**（実装中に踏んで直した）
- **本番で実測済み**: 10個のカルーセル全てで、右端に寄せると fill が `100%`（120px）。
  スクロール余地が無い1個はバー自体が出ない。ベストセラーにもバーが1本出ている
- Figma: `Carousel Progress` を 06 Carousels に追加し、9個の Carousel Dots インスタンスを差し替え。
  ベストセラー区画3つにも追加。`Carousel Dots` は description に「旧・使わない」と書いて残してある

### 3. 管理画面「メニュー管理」の状態UI — 3案を出して**選択待ち**

天真の指摘「『売り切れ』と『表示する』が違う UI だし、トグルが何を示すかなどがわかりにくい」。

3案を `.claude/verification/2026-09-13-admin-state/admin-state-3plans.html`（`.png`）に作った。
**まだ実装していない。天真がどれを選ぶか待ち。**

| 案 | 形 | 弱点 |
|---|---|---|
| **A（AI推奨）** | 状態チップ1つ（販売中／売り切れ／非表示）。押すと 販売中⇄売り切れ。非表示は編集パネルへ | 一覧から直接「非表示」にできない |
| B | 今の2つのトグルにラベル（公開／販売）を付けるだけ | 幅を食う・押し間違えやすい・関係は不明瞭のまま |
| C | 「販売中 / 売り切れ / 非表示」の3択セグメント | 幅を大きく取る・誤タップで非表示にする事故 |

### 4. 管理画面の状態UI = 案A に決定（PR #75、本番反映済み）

天真が案Aを選択。「売り切れチップ＋公開トグル」→ **状態チップ1つ**（販売中／売り切れ／非表示）。

- 販売中 ⇄ 売り切れ は1タップ。非表示は押しても変わらず、戻すのは編集パネルの「公開する」
- **一覧からの公開・非公開の切り替えは廃止**（営業中の操作ではないので編集パネルに集約）
- 文言・判定は `lib/soldOut.ts` の `MENU_ITEM_STATE_LABEL` / `menuItemState()` に1か所化
- Figma: `Sold Out Chip` → **`Menu State Chip`**（State=販売中/売り切れ/非表示）に作り替え、
  `Admin Menu Row` 4種（通常・Mobile・(Sold Out)・(Mobile)(Sold Out)）からトグルを外して状態チップに差し替えた

### 5. 進捗バーの仕上げ（PR #76、本番反映済み）

天真の指示2件:
- 「ブラックで目立ちすぎる」→ 地 `bg/tertiary`・埋まる側 `text/tertiary`（墨をやめた）
- 「上のコンテンツとの間が16pxで詰まる／6pxは太い」→ **間隔 24px**（`mt-20` ＋ バーの上パディング4）、**高さ 4px**（角丸2）

Figma も同じ寸法・同じ色に揃えた。Figma 側は親（Menu Section）の gap 16 が乗るので、
`Progress Wrap` の上パディングは **4**（16＋4＋バーの4＝24）。**コードの mt-20 と数字が違うのは構造の差**で、
見た目はどちらも 24。

ついでに直したもの: `/dev/ui` のサムネイルが `basePath` を通っておらず、本番のギャラリーで画像が割れていた（`asset()` 経由に）。

---

# セットドリンク割引（2026-09-13・本番反映済み、設定は OFF）

仕様: `docs/specs/set-drink-discount.md`。PR #78。洋輔さんの依頼を天真が中継:

> 食事とドリンクをご注文でドリンク200円引き。フードの数分だけドリンクの金額 -200円
> （2名でパンケーキ1・ドリンク2 なら、ドリンク1つ分だけ200円引き）自動適用できれば助かります

## 決まり

- 対象の杯数 = **min(フードの点数, ドリンクの点数)**。1杯あたり既定200円、**そのドリンクの単価を超えない**（安い順）
- フード／ドリンクの判定は**カテゴリーの区分**（`categories.category_type`）
- 税は**割引後**に掛ける。既定は**店内のみ**（設定でテイクアウトにも広げられる）

## 実装

| 場所 | 中身 |
|---|---|
| `supabase/set_drink_discount.sql`（新規） | `stores.set_drink_*` 3列、`orders.discount_amount`、`set_drink_discount_for()`、`place_order` の差し替え、`save_set_drink_setting()`（manager 限定） |
| `lib/setDrink.ts`（新規） | 文言・設定の読み書き・**サーバーと同じ計算**（画面に出すため）。金額の正は DB 側 |
| `app/cart/page.tsx` / `app/complete/page.tsx` | 「セットドリンク割引 −¥200」の行（付くときだけ）。合計は割引後 |
| `lib/store.ts` / `lib/history.ts` | 履歴のスナップショットに `discountAmount` |
| `app/admin/(protected)/menu/set-drink/page.tsx`（新規） | 管理画面「メニュー ＞ セットドリンク」。使うか／割引額／テイクアウト／対象カテゴリーの確認 |
| `components/admin/nav/MenuAccordionNavItem.tsx` | サイドバーのメニュー配下に「セットドリンク」 |

## ⚠ 金額の計算をサーバー側に移した

これまで `orders.total_amount` は**お客様の画面が計算した値をそのまま保存**していた。
割引が絡むと金額がずれる余地ができるため、`place_order` が明細と設定から計算し直すようにした。
引数 `p_total_amount` は互換のため残してあるが**使っていない**。割引 0 のときの金額は従来と同じ結果になる
（本番で「嘘の合計 99999 を渡しても正しい 3,146 円で保存される」ことを確認済み）。

## 踏んだ罠

**`STABLE` の関数の中では `CREATE TEMP TABLE` が使えない**（0A000）。最初の実装が一時テーブルを使っており、
本番に流した直後の検証で**「割引を ON にした瞬間に注文が通らなくなる」**状態だと判明した。CTE だけで書き直して修正。
→ **SQL を流したら、OFF のままにせず「ON にした状態」もトランザクションの中で必ず試すこと。**

## 残り

- 天真・洋輔さん: 管理画面「メニュー ＞ セットドリンク」で **ON にする**と効きはじめる。割引額（既定200円）とテイクアウトの扱いもここで
- お客様に見える文言「セットドリンク割引」は天真の確認待ち（CLAUDE.md 3章）
- レジ画面に割引の内訳を出すのは未対応（`total_amount` は割引後なので会計は正しい）
- 洋輔さん向けの共有資料は未作成

---

# 2026-09-13（夜）: 注文が消えない件とタブのズレ（洋輔さんの指摘2件）

PR #80。資料は `docs/share/2026-09-13-set-drink-and-fixes.html`。

## 1. 厨房・レジから注文が消えない

**原因は3つ重なっていた。**

1. 9月2日の検証注文2件が会計されないまま残っていた
2. **厨房もレジも日付で絞っていなかった**（主因）。status だけで絞っていたので、
   会計されなかった注文は**何日でも残る**。実際に11日間レジに出ていた
3. 席の名前の変更で、**いまは無い席名**で出ていた。ボックス席のコードが A → B に
   変わった際に古い席の行が消え、`orders.table_id` が NULL に。卓名は注文時点の
   スナップショットなので「ボックス席 A-1」のまま残り、「テーブル席 A-1」と並んで
   **A-1 が2つある**ように見えた（卓名を残すのは仕様どおり）

**対応**: 厨房・レジとも `business_date = 今日` で絞る（`businessDateToday()` を
`lib/dateFormat.ts` に追加。DB の `orderly_business_date()` と同じ定義＝JST の暦日）。

⚠ **日をまたいだ会計はできなくなる。** ブランチ営業なので問題ない想定。深夜営業になったら範囲を広げる。

**本番のリセット（天真の指示「全部消してもよい」）**: 注文4件を明細・オプション・印刷ジョブごと削除した。
消したものの記録:

| 日時（JST） | 卓 | 中身 | 金額 |
|---|---|---|---|
| 09-02 16:51 | ボックス席 A-1 | パンケーキ プレーン ×2 | 3,033 |
| 09-02 16:53 | ボックス席 A-1 | パンケーキ プレーン ×1 | 1,540 |
| 09-10 13:17 | テーブル席 A-1 | シーザーサラダ / グリーンサラダボウル | 2,662 |
| 09-13 12:54 | テーブル席 A-1 | 9品（検証） | 13,794 |

削除後: orders / order_items / order_item_options / print_jobs すべて 0 件。

## 2. タブを押した先がずれる・押せない

**原因も3つ重なっていた。**

1. **貼り付き位置が `SCROLL_OFFSET = 118` の決め打ち**。実際と違うと、その差だけ行き過ぎる／届かない
2. **現在地の判定が IntersectionObserver の「帯に入っているか」**だった。タブで移動して止まると
   目的の区画の上端がちょうど境界線に重なり、判定から外れて**前の区画が勝つ**。
   → 「ピザを見ているのにパスタが選択中」の正体
3. **スクロール中に現在地が次々変わり、TabNav が横スクロールで追従**していた。
   狙ったタブが動くので押せない・隣を押してしまう

**対応**: `hooks/useSectionSpy.ts`（新規）に共通化。
- 貼り付き位置は **DOM から測る**（sticky の `top` ＋ 実高さ）
- 現在地は「**上端が境界線を越えた最後の区画**」。真偽ではなく位置で決める
- タブを押したら**すぐ選択を移し、落ち着くまで監視を止める**（指で動かしたら即解除）
- トップ（`app/order/page.tsx`）とカテゴリー一覧（`app/order/[category]/page.tsx`）の両方で使う

**本番で実測**: 実機と同じ幅（390・touch）で13個のタブを順にタップ。
**12個がズレ 0px** で着地し、タブの選択も一致。残る1個は最下段の「キッズ」で、
ページの最下端でそれ以上スクロールできないため（選択は正しい）。
スクリプトは scratchpad の `verify-tabs.mjs`（リポジトリには入れていない）。
証跡: `.claude/verification/2026-09-13-fixes/prod-tab-{pasta,pizza}.png`

## 次に同じ症状が出たら

- 「画面から消えない」→ まず `business_date` で絞っているかを疑う。status だけの絞り込みは残り続ける
- 「タブ・アンカーがずれる」→ 貼り付き位置の決め打ちと、境界が重なる判定を疑う

---

# レジのレシートプリンタ（2026-09-14・調査のみ。実装は判断待ち）

洋輔さんの依頼: Squareだと金額だけのレシートしか出ないので、GOOD ORDERから明細入りの
レシートを出したい。キッチンのEPSONから出すのは場所的におかしいので、レジにある
**スター精密 TSP100LAN（TSP143LAN、192.168.1.3）** をレシート専用に使えないか。

資料: `docs/share/2026-09-14-receipt-printer.html`

## 調査結果

### ① 用途ごとの出し分け → できる

`print_jobs` に用途（オーダーシート / レシート）を持たせ、プリンタごとに受け口の
トークンを分け、`claim_print_job(store_id, 用途)` にすれば済む。いまは1店舗1キュー・
1プリンタ前提（`claim_print_job(p_store_id)` が用途を見ていない）。

### ② TSP100LAN を今のまま繋ぐ → **できない**

**理由**: いまの印刷は EPSON の Server Direct Print、つまり**プリンタ側が数秒おきに
こちらへ HTTP で聞きに来る**方式。だから店舗のルーターに穴を開けずに済んでいる。
初代 TSP100（futurePRNT）には「自分から聞きに行く」機能が無く、**LAN内から生データを
送り込まれるのを待つだけ**（TCP 9100）。Vercel 上の GOOD ORDER から 192.168.1.3 には届かない。

回避策も全部塞がっていた:

| 手 | 結果 |
|---|---|
| Star CloudPRNT（EPSONと同じ pull 方式） | 対応は mC-Print / mC-Label / TSP143IV / TSP654II・700II・800II・SP700 の CloudPRNT モデルのみ。**初代 TSP100 は対象外** |
| 管理画面のブラウザから直接 192.168.1.3 へ | HTTPS のページから店内 HTTP へはブラウザが遮断（mixed content / Private Network Access）。**どのみち不可** |
| iPad から AirPrint | **TSP100 は AirPrint 非対応** |
| PC にプリンタ登録して `window.print()` | 技術的には可能だが、**レジは iPad**（天真確認済み）なので使えない |

### 3案（洋輔さんの判断待ち）

1. **レジにも EPSON を置く**（おすすめ）— 今の仕組みがそのまま2台に増えるだけ。数万円
2. **TSP100LAN を活かし、店内に中継役（Raspberry Pi 等）を置く** — 1万円台。
   GOOD ORDER に聞きに行って 192.168.1.3:9100 へ流す常駐ソフトを新規に作る。
   止まったとき用の見張りも要る（`printer_status` の仕組みを流用できる）
3. **Star TSP143IV に買い替える** — CloudPRNT の受け口を新規実装。1より手間が増える

**どの案でもアプリ側（出し分け＋レシートの版面）は共通で必要。** 違うのはレジに置く機械だけ。

## 未確定（洋輔さん確認待ち）

- どの案で進めるか
- レシートに**インボイス（適格請求書）の登録番号**を載せるか（載せるなら T+13桁が要る）
- レシートを出すタイミング（「会計済みにする」で自動 / ボタン）
- Square のレシートと2枚になる件の運用

## 注意

厨房伝票は**金額を刷らない**決定なので、レシートは版面を新規に作る（`lib/receipt.ts` の
`ticketXml` とは別物）。金額は `orders.total_amount`（割引後）と `discount_amount` を使う。

---

# 会計後に注文が厨房へ戻ってくる不具合（2026-09-14・修正済み）

PR #83。資料: `docs/share/2026-09-14-kitchen-paid-order-fix.html`
**洋輔さんが送ってきた iPad の操作動画（33秒）から発見した。**

## 症状

1. 厨房で「すべて提供済みにする」→ 厨房から消える（正しい）
2. レジで「会計を確定」→ レジから消える（正しい）
3. **厨房に戻ると、提供も会計も終わった注文が復活している**

## 原因

`orders.status` が1つしかなく、**会計時に `served` が `paid` に上書きされて
「提供済みだった」記憶が消える**。厨房は `.not("status","in","(served,picked_up)")`
なので、`paid` になった瞬間に条件をすり抜けて戻ってくる。

`paid` を除外しないのは 2026-08-26 の意図的な決定（テイクアウトで先に会計されたとき、
未調理の注文が厨房から消えて「お金は払ったのに商品が出てこない」を防ぐため）。
その副作用だった。

## 修正

`app/admin/(protected)/kitchen/page.tsx` に `isFinished()` を足し、
**`status === 'paid'` かつ全明細が `cooking_status === 'done'`** の注文を
グルーピング前に落とす。調理の実態は明細で見る。

**8/26 の決定は崩していない。** 1品でも done でなければ残る。

> 本筋は `status` を「調理・提供の進み具合」と「会計の状態」の2つに分けること。
> それは注文まわりの作り替えになるので今回はやっていない。**次に似た症状が出たら、
> まず「状態の欄が1つしかない」ことを疑う。**

## 本番で確認（すべてロールバック済み。データは変えていない）

| ご注文 | 調理 | 修正前 | 修正後 |
|---|---|---|---|
| 09-14 14:05 テーブル席 A-1 ¥4,169 | 3/3 done | 厨房に出る | **出ない** |
| 09-14 14:43 テーブル席 A-1 ¥2,178 | 1/1 done | 厨房に出る | **出ない** |
| 会計済み＋1品未調理（取引内で再現） | 2/3 done | 出る | **出る**（8/26 の決定を維持） |

¥4,169 + ¥2,178 = ¥6,347 は動画のレジ画面と一致。

## 受渡画面の同種の穴 → 2026-09-14 に対応済み（下記の章）

## 動画の解析のしかた（今回やったこと）

`ffmpeg` で中身を読める。音声を文字に起こす手段は持っていない。

```
ffprobe -v error -show_entries format=duration -show_entries stream=codec_type,width,height ...
ffmpeg -i in.mp4 -vf "fps=1/3,scale=440:-1,tile=4x3:padding=6" sheet.jpg   # 一覧シート
ffmpeg -ss 27.5 -i in.mp4 -frames:v 1 -q:v 2 frame.jpg                     # その瞬間を1枚
```
一覧シートで当たりをつけて、怪しい区間を `-ss`＋`fps=1` で細かく切り直すのが速い。


---

# 受渡画面の穴（2026-09-14・修正済み）

PR #85。SQL: `supabase/pickup_completed.sql`（本番適用済み）。
厨房の件（PR #83）と**同じ原因**。天真の指示で続けて塞いだ。

## 症状

テイクアウトを「提供済み」にしたあとに会計すると `status` が `served` → `paid` に
上書きされ、**まだ渡していないのに受渡画面から消える**（「お金は払ったのに受け取れない」）。

## なぜ画面だけ直すと駄目だったか（重要）

最初に考えた「受渡画面が `status IN ('served','paid')` を拾う」だけの修正は**成立しない**。
受渡完了が `status='picked_up'` を書くので、**今度は `paid` が消えてレジに未会計として
戻ってくる**。金額に関わる分、直す前より悪い。**状態の欄を奪い合う限り、画面を1枚直すたびに
別の画面が壊れる。**

## 実際にやったこと（会計には触っていない）

**「渡したか」を `orders.picked_up_at` に分離した。**

- 受渡完了は RPC `mark_order_picked_up(p_order_id, p_expected_updated_at)` に集約
  - **`status='paid'` のときは status を書き換えない。** `picked_up_at` だけ入れる
  - それ以外は従来どおり `status='picked_up'` ＋ `picked_up_at`
  - `order_type='takeout'` / 二重受渡 / 楽観ロック（updated_at 一致）も RPC の中で見る
  - 戻り値は更新後の `updated_at`。**NULL = 競合または受渡済み**
- 受渡画面は `picked_up_at IS NULL` で絞る。`status='paid'` は「先に会計だけ済ませた
  （＝まだ作っていない）」を含むので、**全品 `cooking_status='done'` を確かめてから出す**
  → 厨房画面（会計済み＋全品done を下げる）と**ちょうど裏返しの条件**。対で覚える
- **RLS は触っていない。** 権限は RPC の中でロール（counter/kitchen/manager）を見る。
  金額・会計の権限（paid）は register/manager のみのまま

## 本番での検証（取引内で作ってロールバック。データは残していない）

| 確認 | 期待 | 結果 |
|---|---|---|
| 受渡待ちに出る件数（A会計済み・B未会計） | 2 | 2 |
| A の受渡完了 → **状態** | **paid のまま** | **paid** |
| A に `picked_up_at` が入る | true | true |
| B の受渡完了 → 状態 | picked_up | picked_up |
| 受渡後の受渡待ち件数 | 0 | 0 |
| 二重の受渡完了 | 弾かれる（NULL） | NULL |
| **レジに未会計として出るのは B だけ** | B のみ | **B** |

`anon` から `mark_order_picked_up` は呼べない（確認済み）。
過去データの埋め漏れ 0 件（`status='picked_up'` の行はそもそも 0 件だった）。

## 残っている本筋

`orders.status` が「調理・提供の進み具合」と「会計の状態」を1つの欄で持っている構造は
そのまま。**会計を `paid_at` のような別の欄に分けるのが本筋**で、そうすれば厨房・受渡の
両方のつぎはぎ（PR #83 の `isFinished()` と、ここの `picked_up_at`）が要らなくなる。
今回やらなかったのは、**レジの会計処理と RLS に手が入る**ため（CLAUDE.md 3章）。
やるなら天真の OK を取ってから。

**次に似た症状が出たら、まず「状態の欄を奪い合っていないか」を疑う。**

---

# レジが割引を無視／完了画面のイラストが割れる（2026-09-15・修正済み）

PR #87。資料: `docs/share/2026-09-15-register-discount-and-logo.html`。洋輔さんの指摘2件。

## 1. レジにセットドリンク割引が反映されていなかった（金額のずれ）

**セットドリンク割引は 2026-09-15 から本番で ON になっている**（店舗が有効化した）。

レジの `selectedData` は明細から `subtotal → tax → total` を計算し直すだけで、
**`orders.discount_amount` も `orders.total_amount` も見ていなかった**。
そのためお客様の画面より高い額をレジに出していた。

| | お客様 | 修正前のレジ | 差 |
|---|---|---|---|
| 内部ID 427f9b（写真の注文） | ¥3,553 | ¥3,773 | +¥220 |

当日該当したのは8件（うち会計済み7件）、差の合計 ¥2,420。洋輔さんに Square との
突き合わせを依頼済み。

**修正**: 合計は **`orders.total_amount` の合計をそのまま使う**。これは `place_order` が
サーバー側で計算して保存した値＝お客様に提示した金額そのもので、ここが正。
消費税は `合計 −（小計 − 割引）` で逆算し、3行が必ず足して合うようにした。
`total_amount` が 0 の古い注文だけ従来計算にフォールバック。
`BillCard` に `discount` を足し、0 より大きいときだけ「セットドリンク割引」の行を出す。

> **教訓: 金額は画面で計算し直さない。** DB に保存された値を出す。
> カート・完了・レジで別々に計算していると、割引のような後付けの要素でかならず食い違う。
> 2026-09-13 に `place_order` をサーバー計算に変えたとき、**レジを直し忘れていた**のが原因。

## 2. 完了画面のイラストが割れていた

`components/ChefSmileIllustration.tsx` が `src="/illustrations/chef-smile.svg"` と
直書きで **`basePath` を通っていなかった**。本番は接頭辞つき（`/yorkys-shukugawa/...`）
なのでドメイン直下を見に行って 404。`asset()` 経由に修正。

実測: `/yorkys-shukugawa/illustrations/chef-smile.svg` → 200 ／ `/illustrations/...` → 404

`/dev/ui` のサムネ（2026-09-13）と**同じ原因で2回目**。
リポジトリ全体を `grep 'src="/'` で調べ、**残りはこの1件だけ**だったことを確認済み。

> **新しく画像を足すときは必ず `asset()` を通す。** `src="/..."` と書いたら本番で割れる。

---

# 税が二重にかかっていた（2026-09-15・修正済み）

PR #89。仕様: `docs/specs/tax-mode.md`。資料: `docs/share/2026-09-15-tax-mode.html`。
洋輔さんの依頼「メニュー価格を税込で入れているのにさらに10%かかる。内税/外税を選べるように。
テイクアウト8%・店内10%」。

## 何が起きていたか

**二重課税。** 店舗は価格を税込で登録していたのに、システムは「価格は税抜」と決め打ちで
どこでも `× 1.1` していた。小計3,430・割引200 のご注文に **3,553円**（本来 3,230円）。
検証注文19件で計 ¥5,477 のずれ。**プレオープン前だったのでお客様への影響は無し**（天真が確認）。

## 直した形

| 場所 | 中身 |
|---|---|
| `stores.tax_mode` | `included`（内税）/ `excluded`（外税）。**既定 `included`** |
| `stores.tax_rate_dine_in` / `tax_rate_takeout` | 既定 10 / 8（軽減税率） |
| `orders.tax_amount` / `tax_rate` | その注文の消費税額と税率。**レシートの税率ごとの内訳（インボイス）に使う** |
| `public.calc_order_total()` | 計算の本体。`place_order` が呼ぶ |
| `lib/tax.ts` の `calcOrderTotals()` | 画面用の写し。**規則を変えるときは必ず両方** |
| `/admin/tax` | 管理画面「税の設定」（manager のみ） |
...
- **既定を内税にした。** `TAX_DEFAULT` も内税。設定が読めなくても二重課税に戻らない
- **消費税を足し算の行に出すのは外税のときだけ。** 内税で行に出すと上から足しても
  合計にならず、二重課税に見える。内税では合計の下に「うち消費税」と添える
- **税率は注文の種別（order_type）で決める。** 1回の注文に店内商品とテイクアウト商品が
  混ざりうるが、**内税では支払額が変わらない**ので作りを複雑にしない。
  必要になったら `order_items` 側に税率を持たせる

## 本番での検証（すべてロールバック済み）

| 設定 | 合計 | 消費税 |
|---|---|---|
| 内税・店内10%（小計3,430・割引200） | **3,230** | うち 293 |
| 内税・持帰8% | 3,230 | うち 239 |
| 外税・店内10% | 3,553 | 323 |
| 外税・持帰8% | 3,488 | 258 |
| `place_order` を実行（店内・割引あり） | 小計2,310 → 合計 **2,110** ／ 税191 ／ 10% | |

## 今回で3回目の同じ失敗

金額の計算が画面ごとにバラバラだったことが、9/15 だけで2件の不具合を生んだ
（レジが割引を無視／税の二重がけ）。**金額は DB に保存された値を出す。画面で計算し直さない。**
いまは `place_order` が計算して `total_amount` / `discount_amount` / `tax_amount` に保存し、
レジはそれを読むだけになっている。**この形を崩さないこと。**

## 残り

- レシートに税率ごとの内訳を刷る（インボイス）。`tax_amount` / `tax_rate` は用意済み。
  プリンタの選定と合わせて（`docs/share/2026-09-14-receipt-printer.html`）
- 外税で運用する場合は、メニューの価格を税抜に入れ直す必要がある（洋輔さんに伝達済み）

---

# ハンバーガーメニューの不具合と、設定画面2つ（2026-09-15・すべて本番反映済み）

PR #92 / #93 / #94。資料: `docs/share/2026-09-15-menu-and-settings.html`。洋輔さんの指摘5件。

## 1. ハンバーガーメニューにカテゴリーが2つしか出ない（PR #92）

**原因**: `app/order/menu/page.tsx` だけ `FOOD_ORDER` / `DRINK_ORDER` という **slug の固定リスト**を持ち、
そこに載っている slug のカテゴリーしか出していなかった。店舗の14カテゴリーのうち一致したのが
`pancake` と `alcohol` の2つだけ。

> **同じ不具合をトップページで 2026-08-26 に直していた。**`lib/orderHome.ts` の
> `pickFoodCategories` に「固定リストとの一致で絞らない」という警告コメントまで残っていたのに、
> **このページだけ直し漏れていた。** 横展開の確認をしていなかった。

**修正**: `orderHomeCategories` / `itemsOfCategory`（トップと同じ関数）で DB から作る。
英語名は `category.caption` を最優先（以前は固定の対応表と slug だけで「FRENCHFLIES」等になっていた）。
**写真が無いカテゴリーでは黒オーバーレイと白文字を使わない**（売り切れの帯と同じ判断）。
商品ゼロのカテゴリーは出さない（行き止まりになるため。トップと同じ基準）。

**アルコールがカードに出ないのは正しい。** ドリンクの**サブカテゴリー**なので DRINK に含まれる。

## 2. テイクアウト（PR #92 の導線 ＋ #94 の画面）

- 導線: テイクアウト対象が0件なら「テイクアウト（準備中）」で押せなくする。`LinkButton` に `disabled` を追加
- 画面: この画面だけ旧デザイン（`Header` / `CartButton` / `FloatingStaffCall` / 自前カード）で
  取り残されていた。カテゴリー一覧と同じ器に統一。空のときは「店内メニューを見る」ボタンを置いて
  行き止まりにしない

## 3. 店舗情報の設定画面（PR #93・新規）

店舗情報が `lib/siteConfig.ts` に直書きで変えられず、**写真は basePath 未経由で404**だった
（イラスト・`/dev/ui` に続き**3回目**）。

- `stores` に `info_image_url` / `address` / `hours` / `holiday` / `phone` / `map_url`
- `/admin/store`。写真はアップロード可。**未設定なら siteConfig の既定値にフォールバック**
- ⚠ JSON-LD と meta は siteConfig のままで連動しない

## 4. スタッフ呼び出しの項目設定（PR #93・新規）

- `staff_call_options` テーブル（読みは全員、書き換えは manager 限定 RPC のみ）
- `/admin/staff-calls`。文言・アイコン・並び・表示/非表示、最大8件。保存は**丸ごと置き換え**
- `staff_calls.call_type` / `call_label` は自由テキストなので**厨房側は変更なし**
- 初期値に今までの3つを入れてあるので、流した時点で見え方は変わらない

## 5. Figma

- **`Nav Sidebar v2`（共通コンポーネント）に4項目を追加**: 印刷状況 / 税の設定 / 店舗情報 / スタッフ呼び出し。
  design-rules 6 の「上書きがズレる」対策として、**21インスタンス全部のラベルを変更前後で突き合わせ、
  ズレ0件を確認してから**確定した。スペーサーが `layoutGrow:1` なので高さは自動で吸収される
- `Store Info / 店舗情報` `Staff Calls / スタッフ呼び出し` セクション新設（PC/SP）
- `注文 / SP` に `Menu — 写真なし（カテゴリー自動一覧）` と `Takeout — 準備中`

### Figma で踏んだこと（次に効く）

1. **複製した行は変数バインドが解決されず黒(0,0,0)になることがある。**
   1行目の `fills` オブジェクトをそのまま代入して直した。`setBoundVariableForPaint` で
   作り直すより、**効いている塗りを写す**ほうが確実
2. **サブセクション内のフレームは「横1列・100px間隔」が規約。**
   `scripts/check-figma.mjs` は x だけで並べて隙間を見るので、**折り返すと全部「重なり」判定**になる。
   見やすさのために格子状に並べ替えたら18件の違反になった
3. 空のテキストに `getRangeFontName(0,1)` は落ちる。`fontName`（mixed でなければ）を使う

---

# 機能設定・注文履歴・退店後の締め（2026-09-15 夜・すべて本番反映済み）

PR #96 / #97 / #98。資料: `docs/share/2026-09-15-settings-and-screens.html`。

## 1. 機能設定（stores.kitchen_enabled / staff_call_enabled）

洋輔さん「厨房にiPadを置かないので一旦この機能を使わないようにできますか」「スタッフを呼ぶも」。
**消すのではなく切る**（あとから戻す前提）。既定はどちらも ON。

- 置き場は **表示設定 ＞ 機能設定**（天真の選択。画面を増やさない）
- 最初「使う機能」という名前にしたが**何の設定か伝わらない**と指摘され「機能設定」に改名
- **厨房 OFF でも伝票の印刷は止めない**（天真の決定）。伝票で注文を受け取る運用になるため
- スタッフ呼び出し OFF は**お客様側の入口を全部消す**（メニュー内の項目と右下のベル）

### スタッフ呼び出しの設定を統合（天真の指示）

独立ページ `/admin/staff-calls` を**廃止**し、機能設定に寄せた。
**ON のときだけ「設定する」ボタンを出し、ポップアップで中身を編集**する。
ON/OFF と中身が別の場所にあると、どちらを触ればいいか分からなくなるため。
旧URLは `next.config.mjs` で `/admin/display` へリダイレクト。

## 2. 注文履歴（お客様が金額を見られるように）

**画面は前からあった。** だが導線が**旧デザインのドロワーにしか無く**、新デザインからは
辿り着けなかった（洋輔さんの「これはすでになかった？」はここ）。

- ハンバーガーメニューに「注文履歴」を追加
- 画面を新デザインに（`OrderHeader` / `BottomViewCartBar` / 見出し EN+JP）。
  カード・空の状態・再注文の確認ダイアログもトークンに寄せ、絵文字アイコンをやめた

## 3. 退店後の注文を止める（lib/visitSession.ts）

**それまで期限がまったく無かった。** 卓とカートは localStorage に無期限で残るので、
退店どころか翌日でも注文できた。

- **レジで「会計済みにする」が押されたら締める**（天真の選択）
- 判定: この端末が出した注文を `get_order_statuses`（anon 可）で引き、**1件以上あって全部 paid** なら終了
- 締まると `VisitClosedGate` が「ご来店ありがとうございました」を出す。
  **二次元コードを読み直すと新しい来店として開き直す**（`beginVisit()` が履歴を消す）
- **通信に失敗したら締めない。** 電波が悪いだけで注文できなくなるのを避ける

> ⚠ **時間での自動終了は入れていない**（天真の選択）。
> **会計を押し忘れた卓は、その端末からずっと注文できる。**
> 必要になったら `lib/visitSession.ts` に「最後の注文から N 時間」を足す。

## 4. 厨房カードが長い品名で崩れる（PR #96）

品名の行に `min-w-0` / `flex-1` が無く `whitespace-nowrap` だけが効いていたため、
長い品名で行が縮まず、状態バッジと「すべて提供済みにする」が枠外に押し出されていた。
**品名だけ truncate、数量・タイミング・バッジは `shrink-0`。**

## Figma

- `Nav Sidebar v2` から「スタッフ呼び出し」を削除（21インスタンスのラベルを突き合わせ、ズレ0件）
- `Staff Calls / スタッフ呼び出し` セクションを **`Feature Settings / 機能設定`** に作り替え
- `Modal / スタッフ呼び出しの設定 — PC` を追加
- `注文 / SP` に `Order History — 新デザイン`
- 検品: 構造・パディング 全ページ問題なし／新しい違反なし・増えた違反なし

### また踏んだこと

**テキストスタイルを当てる前に、そのスタイルが使うフォントを読む必要がある**（Barlow Medium で落ちた）。
`getLocalTextStylesAsync()` を回して全部 `loadFontAsync` しておくのが手っ取り早い。

**JSX コメントの `*/}` 閉じ忘れを再発させた。**（2026-09-12 に一度やっている）
`{/* … */` で終わると TS1005 が離れた行で出るので原因が分かりにくい。

---

# 細部の調整（2026-09-15 深夜・すべて本番反映済み）

PR #100。資料: `docs/share/2026-09-15-refinements.html`。天真の指示4件。

## 1. 店舗情報の項目を可変に（stores.info_rows）＋ Places 連携

項目が4つ固定で、空にすると既定値に戻る＝**「載せない」ができなかった**。
`info_rows`（jsonb の配列 `[{label, value, icon}]`）に移し、追加・削除・並べ替えを
できるようにした。保存は `save_store_info_v2()` で**丸ごと置き換え**。

- **旧列（address/hours/holiday/phone）は消していない。** 読むのをやめただけ。
  列の削除は取り返しがつかないため
- 店舗名・写真・地図URLは行にしない（扱いが違う／地図は自動で入るため）
- 項目は最大12個（AI 判断）

**Places 連携**: `app/api/admin/places/search/route.ts`。GOOD LOOP と同じ Text Search。
GOOD_ORDER にはサーバー側の Supabase クライアントが無いので、
**画面から送るアクセストークンを `auth.getUser(token)` で検証し、manager だけ通す**。
鍵 `GOOGLE_PLACES_API_KEY` はサーバー専用（`NEXT_PUBLIC_` を付けない）。
...
`LinkButton` の `p-16` が左右にも効いて長いラベルが枠に張り付いていた。
`px-12 / py-16` にし、**`note` プロップを足して「準備中」を一段小さく別に出す**。

**導線の判定は確認済み**: `menuItems.some(i => i.isTakeout)`。`menuItems` は
`is_available=true` のみなので、**テイクアウト画面の絞り込みと同じ条件**。
1件でも追加すればアクティブになり、「押せるのに中身が空」は起きない。

## 3. 注文履歴

- 今日／過去の区分を削除（1人あたり1来店ぶんしか入らない）
- 再注文ボタンと確認ダイアログを削除（`doReorder` 一式ごと）
- **1行1品**。品名・数量・金額を列で揃え、オプションと「食後」は下に小さく
- **「調理中／提供済み」のタグを削除。** 厨房画面を使わない運用では動かないまま
  残るため。**状態の取り込み自体は続けている**（将来また出すときのため）

## 4. カート

`QuantityStepper` には元から `min` があったのに `CartItemRow` が渡しておらず、
個数1でマイナス→0で行ごと消えていた。`min={1}` を渡して押せなくした。

## また踏んだこと

**三項演算子の `) : (` の直後に JSX コメントを置くと壊れる。**
そこは「1つの要素」しか置けない。コメントは三項の外に出す。
（`*/}` 閉じ忘れと合わせて、JSX コメントで詰まるのは今日3回目）

---

# 障害: カートに消えた商品が残ると注文できなくなる（2026-09-15 深夜・修正済み）

PR #103。**洋輔さんが遭遇した致命的な不具合。**

## 症状

「注文を確定する」を押すと必ず
「通信エラーのため、ご注文を送信できませんでした。もう一度押してください」
と出て、**何度押しても進めない**。

## 原因

**カートに、いまメニューに無い商品が残っていた。**
カートは端末の localStorage に残り続けるので、商品を削除・非公開にすると
その行を抱えたまま注文することになる。`place_order` は
`order_items_menu_item_id_fkey` の外部キー違反で弾く。

ところが画面側は **売り切れ以外の失敗をすべて `"failed"` = 「通信エラー」に
まとめていた**ため、

- 再送しても永久に直らないのに「もう一度押してください」と案内し続ける
- しかも `placeOrder().catch(() => ({reason:"failed"}))` で**例外を握りつぶして
  いた**ので、コンソールにも何も残らない

**この2つが重なって「原因が分からないまま永久に詰む」状態を作っていた。**

## 調査で分かったこと（次に効く）

本番で place_order を場面ごとに叩いて切り分けた（すべてロールバック）。

| 場面 | 結果 |
|---|---|
| オプションつき / 提供タイミング / テイクアウト / 卓IDつき | すべて OK |
| **消えた商品がカートに残っていた** | **外部キー違反** ← 原因 |

> **REST 経由で place_order を試すと本番に実データが入る。** 検証で2件作ってしまい
> 削除した。**切り分けは SQL のトランザクション内（ROLLBACK）でやること。**

## 修正

- `lib/soldOut.ts` に `unavailableIdsIn()` と `isUnavailableError()`
- カートで、いまメニューに無い行を**売り切れと同じ見た目**で示し、ボタンを止める。
  **メニューを読み終えるまでは判定しない**（`menuDataStore.loadedAt` を見る）
- 外部キー違反・明細やオプションの検証エラーを `"unavailable"` として通信エラーと分け、
  **再送しない**
- 文言を「赤く表示された商品を削除して、もう一度お試しください」に
- **カート側で例外を握りつぶすのをやめた**

## 教訓

**「失敗」を1種類にまとめない。** 再送で直るもの（通信）と、直らないもの（中身が
今のメニューと合わない）は、お客様への案内がまるで違う。
まとめた結果、**直らないものに対して「もう一度押してください」と言い続けた。**

---

# Vercel の環境変数の警告（2026-09-15・未対応）

洋輔さんのスクリーンショットで判明。Vercel が2件に「Needs Attention」を出している。

| 変数 | いまの種別 | 問題 |
|---|---|---|
| **`SUPABASE_SERVICE_ROLE_KEY`** | Non-sensitive（平文） | **RLS を全部素通りする最強の鍵**。ダッシュボードを見られる人に値が見える |
| `CRON_SECRET` | Non-sensitive（平文） | 同上（影響は小さい） |

`NEXT_PUBLIC_*` が平文なのは**正常**（元から公開前提）。

**お客様のブラウザには漏れていない**ことは確認済み
（使用箇所は `app/api/print` / `app/api/daily-report` / `lib/dailyReport.ts` のみ。
ビルド成果物にも含まれない）。

**対応**: Vercel のダッシュボードで Sensitive に付け替える（いったん削除して入れ直す）。
**鍵の値を扱う作業なので AI はやらない。天真の作業。**

---

# 注文が通らない ─ **本当の原因は卓だった**（2026-09-16・解決）

ひとつ上の「カートに消えた商品が残る」は**誤診**だった。洋輔さんから
「パンケーキ プレーン（いまメニューにある商品）で同じエラーが出る」と指摘されて発覚。

## 本当の原因

本番の `postgres_logs` を引いたら、失敗は**全件これ1つ**だった。

```
insert or update on table "orders" violates foreign key constraint "orders_table_id_fkey"
```

（9/15 の 08:21・09:33・14:17・14:18・14:54・14:55・14:56。`edge_logs` では 409）

- 席設定の保存（`save_table_layout`）は、**一覧に無い卓を DELETE する**
- 9/8 に「A-1」を「テーブル席 / ボックス席」へ組み替えたとき、古い `tables` の行が消えた
- お客様のスマホはカート（`orderly-cart`）に **その時の `tables.id` を持ち続ける**。
  二次元コードから入り直さないかぎり消えない
- → その端末は**恒久的に注文できない**

商品は最初から一切関係なかった。

## 直したもの（PR #105 / `3d44e56`）

- `supabase/order_stale_table_id.sql`（本番適用済み）— `place_order` は卓の行が無いとき、
  **同じラベルの卓に付け替える**。無ければ `table_id` を NULL にして `table_label` だけ残し、
  **注文は通す**。検証: 消えた卓ID + "テーブル席 A-1" → 正しい A-1 の実IDに復帰
- レジ（`app/admin/(protected)/register/page.tsx`）— `table_id` が無い注文を
  `table_number`（現状すべて 0）で束ねると**別のお客様の伝票が合流する**。ラベルを先に見る。
  **SQL と対で入れること。片方だけ入れると会計が壊れる**
- `lib/soldOut.ts` の `isUnavailableError` — `order_items` の `menu_item_id` 違反だけに絞った

卓を参照する外部キーは `orders` だけ（`staff_calls` はラベルしか持たない）。他に同じ壊れ方はしない。

## 教訓

**エラーコードだけで原因を決めない。** `23503` は「外部キー違反」であって
「商品が存在しない」ではない。`orders` には `store_id` / `table_id` の外部キーもある。
**制約名まで見る**か、**サーバーのログを引く**。

今回、画面の症状（カートに商品が残っている）から筋を立てて、コードを読んだだけで確定させた。
`postgres_logs` を最初に引いていれば1手で分かった。**推測で直す前に、本番のログを見る。**

しかも誤った案内（「お取り扱いが終わった商品を削除してください」）は、
**やっても直らない対処をお客様に指示し続けるもの**だった。原因が曖昧なときに
断定的な文言を出すと、黙って失敗するより害が大きい。

---

# 商品詳細の下部バーの重なり ／ おすすめカードの写真なし対応（2026-09-16）

## 1. 下部バーが重なっていた（バグ）

オプションのある商品（ドリンク＝HOT/ICED）を 390px 幅で開くと、
**マイナスボタンがカートアイコンに重なっていた**（洋輔さんが発見）。

実測: 390 − 左右padding 32 = 358 に対し、
カート 48 ＋ gap 12 ＋ ステッパー 124 ＋ gap 8 ＋ CTA **190（固定）** = 382。**24px 超過**。
この段は `justify-end` なので、あふれた分が**左へ出てカートアイコンに重なる**。

文字は「カートに入れる ¥550」で実測 136px しかなく、**190px 固定が単に広すぎた**。

直し方: CTA を `flex-1 min-w-0` にして余白いっぱいまで、Figma の幅（190 / 154）を `max-w` で頭打ちに。
390px では 166px に収まり、余白 12px が戻った。ステッパーに `shrink-0`、
ラベルに省略記号（320px のような極端に狭い端末向け）。

## 2. おすすめカードに写真が無い場合（設計）

実データ: **画像なしはドリンク19件中13件だけ**。他カテゴリは全件あり。
つまり**写真あり・なしは既に混在している**（抹茶ラテなど6件は写真あり）。

天真の決定:
- 写真が無いときは**画像の枠を残したまま、カテゴリのタグ色を地にして商品名を大きく置く**
- **価格は写真の有無にかかわらず必ず出す**（それまでカードに価格が無かった）

「1件も写真が無いときだけ別の形にする」という分岐は**入れていない**。
同じカードで両方成立するので、場合分けが要らないのが正しい。

副次的に直ったもの: **写真の無い売り切れ商品に SOLD OUT の帯が出ていなかった**
（「空箱に帯だけだと画像なしに見える」ため写真ありだけに出していた）。
地に色が付いた今は帯を出してよいので、売り切れのドリンクが売れるように見える状態が解消した。

検証（1400px / 390px）: 写真あり・なし・説明あり・なし・売り切れ を混ぜて
**全カード 493px で高さが揃い、価格の位置も全て同じ**。

⚠ Figma には写真なしのバリアントが無い。**天真が別途デザインを起こす予定**（2026-09-16 時点）。

---

# 商品詳細をハーフモーダルにする（2026-09-16・天真の決定）

## きっかけ（洋輔さんの報告）

おすすめから商品をたどると、**× を押した回数だけしか戻れない**。
「一瞬トップが映ってから直前の商品に戻る」を4回くり返さないと一覧に帰れなかった。

原因は `openItemDetail()` が**毎回 `history.pushState()` していた**こと。
おすすめを1回たどるごとに履歴が1枚増え、`close()` は `history.back()` を1回しか呼ばない。

## 決めたこと

詳細は「一覧の上に重ねた**1枚のシート**」。**履歴も常に1枚**にする。
おすすめから別商品へ移るときは `replaceState`（`openItemDetail(id, { replace: true })`）。
何回たどっても × 一発、ブラウザの戻る・スワイプバックも一発で一覧に戻る。

さらに、全画面 × 右上の × ではハンバーガーメニュー（AppDrawer）と器が同じで主従が分からないため、
**下から出るハーフモーダル**に変えた（天真の決定）。

| 決めた点 | 値 |
|---|---|
| 高さ | `calc(100% - 60px)`。既存の `.bottom-sheet`（スタッフ呼び出し・店舗情報）と同じ「上に60px覗かせる」 |
| 出入り | 下から 380ms `cubic-bezier(0.32,0.72,0,1)` / 下へ 220ms `ease-out`（StaffCallSheet と同数値） |
| 閉じ方 | 右上の ×／下へスワイプ（120px 超 or 速い弾き）／グレーの幕をタップ |
| つまみ | 出す。**KV写真の上ではなく写真の上の帯**に置く（明るい写真で見えなくなるため） |
| おすすめ遷移 | いったん下へ引っ込めて、新しい商品で出し直す |

## 実装で気をつけた点

- **幕は `fixed inset-0` にしない。** `left-0 right-0 top-0 h-viewport`（dvh）で取る。
  `inset-0` だと iOS でアドレスバーが引っ込んだとき下部バーの下に隙間が空く（過去に踏んだ罠）
- **スワイプは中身が先頭にあるときだけ始める**（`scrollTop > 0` なら無視）。
  そうしないと読み下げ中の指の動きでシートが閉じる
- `touchmove` を `preventDefault` したいので、React の `onTouchMove` ではなく
  `addEventListener(..., { passive: false })` で付ける
- `visible` は**次のフレームで**立てる。同フレームで true にすると差分が生まれずアニメが再生されない

## 表示速度について（洋輔さんの懸念への回答）

「以前は速度の兼ね合いで別ページにした」という記憶は**逆**。
もともと `/order/item/[id]` への**ページ遷移だったのをやめて**オーバーレイにしたのが今の形
（戻るとスクロール位置が飛ぶ／閉じるアニメ中に一覧が居ない、の2つが構造的に直らなかったため）。
URL は `pushState` / `replaceState` で直接書き換えていて **RSC の往復すら起きない**。
今回の変更は CSS とレイアウトだけなので、速度は一切犠牲になっていない。

## 検証（390px・ローカル）

| 確認 | 結果 |
|---|---|
| おすすめを3回たどる | 履歴 10 のまま固定（以前は 1 ずつ増えていた） |
| × を1回 | `/order?item=…` → `/order`。一発で一覧 |
| グレーの幕をタップ | 閉じる |
| 下へ 220px スワイプ | 閉じる |
| 下へ 60px で離す | 閉じずに戻る |
| 指への追従 | 60/120/160px に 1:1。幕も 0.5 → 0.30 に連動 |

⚠ Figma にハーフモーダルの詳細フレームは無い。**天真が別途デザインを起こす予定**。
`.page-slide-in-right` / `.page-slide-out-right` は利用元が無くなったので削除した。

## 追補（2026-09-16・実機確認後の2点）

洋輔さんの実機確認「スワイプもハーフモーダルの切り替えも違和感なし」を受けて、2点だけ直した。

**1. シート上部の隙間をなくした**
つまみを「写真の上の帯」に置いていたため、シート上端と写真の間に約20pxの白い帯が残っていた。
写真をシート上端まで敷き詰め、**つまみは写真の上に白抜きで重ねる**（影 `0 1px 3px rgba(0,0,0,.35)` で
明るい写真でも沈まないようにする）。写真の無い商品は白だと見えないので、そのときだけ枠線色にする。
検証: シート上端 60px / 画像上端 60px = **隙間 0**。

**2. 左上に戻る矢印**
おすすめでたどったとき、ひとつ前の商品に戻れなかった（× は一覧まで閉じるため）。
右上の × と**同じ部品・同じ大きさ(48)**で左右対称に置いた。

⚠ **これは `history.back()` ではない。** 履歴は1枚に固定してあるので（× が一発で閉じるため）、
たどってきた並びを `trail` という state で自前に持ち、そこから取り出している。
シートを閉じたら `trail` も捨てる（次に開いたときに矢印が出ないように）。
...
## 追補2（2026-09-16・致命的な抜けと Chrome の崩れ）

**1. 詳細シートに価格が出ていなかった（致命的）**

オプションのある商品だけ「カートに入れる ¥550」の文字に混ざって出ていただけで、
**オプションの無い商品（ハンバーガー・パンケーキ等）は価格を一度も見ないままカートに入る**状態だった。
洋輔さんが本番で発見。プレオープン中の実害は不明。

直し方:
- 商品名の下に `type-en-price-l`（22px）で価格を出す。**選んだオプション込みの単価**なので、
  オプションを変えるとここも変わる
- CTA は**オプションの有無にかかわらず金額つき**に統一（`カートに入れる ¥1,340`）。
  数量をかけた「いま入れる分の合計」なので、押す前に必ず金額が見える

⚠ 390px では CTA の左右の余白が **8.5px** しかない（幅166 / 文字149）。
はみ出しは省略記号で防いでいるが、Figma で詰めるならここ。

**2. Chrome(iOS) で下部ツールバーが引っ込むとシートの下に隙間**

ツールバーが出ているときは正常、引っ込むと背面の一覧が下から覗く、という出方だった。

⚠ **`100dvh` を信用しない。** Chrome(iOS) では実際に見えている高さとずれる。
`window.visualViewport` の `height` / `offsetTop` をそのまま器に当てる。
無い環境では従来どおり `h-viewport`(dvh) が効くようにフォールバックを残した。
`scroll` は指を動かすたびに飛んでくるので、**値が変わったときだけ** state を更新する
（毎回更新するとシート全体が再描画されてスクロールが重くなる）。

検証（390px）: シート上端 60 / 下端 844 / 画面高さ 844 → **隙間 0**。

## この日踏んだ罠（3回目）

JSX コメントの閉じを `*/` と書いて `*/}` を忘れた。**TS1005 は離れた行に出る**（今回は
実際の誤りが 449 行なのに 453 行で報告された）。同じ日に、JSX の**タグの属性の中に**
`/* */` を書く誤りも踏んだ（属性の並びは JS ではないので書けない。タグの外に `{/* */}` で出す）。

---

# 一覧の「カートに入れる」統一 ／ TOPの下部バー ／ 詳細シートのCTA切替（2026-09-16）
...
**全一覧で「ステッパー＝下書きの数量 / カートに入れる＝確定」に統一した。**
TOP のカルーセル（MenuCardM）だけが先にこの形だったのを、カード・行のすべてへ広げた。
下書きは `hooks/useDraftQuantities.ts` に共通化（TOP・カテゴリ一覧・テイクアウトで共用）。

⚠ **横並びは入らない。** 「カートに入れる（小）」108 ＋ gap 8 ＋ ステッパー（小）84 = **200**。
TOP のカルーセルカードは幅200でちょうど収まるが、
- グリッドのカード（`MenuCard`）は **171** → 縦に積む
- 行（`MenuListRow`）は右側を **108px の列に縦2段**（横並びだとサムネ付きで商品名の幅が残らない）

天真の決定（2026-09-16）で**縦積み**。

`MenuListRow` の「＋」（PlusButton）は廃止。行は常にステッパー＋ボタンを出す。

## 2. TOP の左下の小さなカートアイコンをやめた

`FloatingCartButton` を削除し、他のページと同じ `BottomViewCartBar`（下部固定）にそろえた。
利用元が無くなったので部品ごと削除。

## 3. 詳細シート: 入れたら CTA が「カートを見る」に変わる

天真の決定:
> 「カートアイコン（個数バッジ付き）を残しつつ、カートを見る と、ひとつのボタンになる。
>   なぜならこの2つのリンク先は同じなので分離してるとおかしい」

下部バーを2枚重ねにして、下の層（`ViewCartButton`）を CTA の位置（left 58%）から
左いっぱい（left 0）へ伸ばしながら入れ替える。
**幅を transform で変えると文字が歪む**ので `left` を動かしている。

**戻るのはシートを閉じたときだけ**（天真の決定）。おすすめで別の商品へ移ったときは
別の商品なので `added` を false に戻す（そうしないと、おすすめの商品を入れられない）。

### 検証（390px・ローカル）

| 確認 | 結果 |
|---|---|
| 一覧の ＋ を2回 | 表示は3個・**カートは空のまま** |
| 「カートに入れる」 | カートに **3個**まとめて入る |
| 詳細で「カートに入れる」 | バーが幅358（＝バー幅いっぱい）の「カートを見る」に。個数バッジ 4 |
| TOP | 下部固定バー（幅358・下端828）。旧FABは無し |


---

# プレオープン当日（2026-09-16）: 裏取り → 営業後に修正2本

- 深夜に「明日がプレオープン」と依頼されたが、実際は**同日 10 時開店**だった（9 時に判明）。
  開店前はコードを入れず、`docs/opening-checklist-2026-09-16.md` だけ渡した。初日は注文 54 件・伝票 54 件すべて印刷、事故なし
- 裏取り: `docs/preopen-verify-2026-09-16.md`（監査 27 件すべて再現。監査より重かったのは
  **厨房の「すべて提供済み」が会計済みを未会計に戻す**こと）
- 営業後に PR #116（店舗側）・#117（お客様側）をマージ。`supabase/print_jobs_recovery.sql` は本番適用済み
- 人が決める項目 9 つと、直していないリスクの運用回避は同じ文書の末尾

## 踏んだ罠
- Fable のウルトラコード（27 件 × 3 体）は 1 時間で利用制限に当たり、途中の結果ごと消えた。
  裏取りはコードを読めば決まるものを agent に投げない。結果は逐次 docs に残す
- 利用制限で dev サーバーも落ちる。`preview_start name=dev` で立て直す
- `.claude/launch.json` はコミットしない（untracked のまま。ブランチ切替の邪魔になる）
