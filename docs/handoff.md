...
- **受渡画面は `status = 'served'` 完全一致**（`app/admin/(protected)/pickup/page.tsx:52`）。
  テイクアウトを「提供済み」にしたあとに会計すると `paid` に上書きされ、受渡画面から消える。
  実際の順番（注文→会計→調理→提供済み）では起きないが、**同じ種類の穴**なので頭に入れておく。
