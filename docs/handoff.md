...
# 検討事項: テイクアウトの事前決済（Stripe）（未着手・2026-09-04 発案）

天真と洋輔の議論から。**マクドナルド方式**——事前にオンラインで決済を済ませ、
店頭では受渡番号で受け取るだけにする。混雑緩和が主目的。
YORKYS BRUNCH では外向けの注文パネルまでは不要。

## 決済手段（2026-09-04 時点で Stripe 公式を確認済み）

「カード決済だけだと日本の若い層が使えない」という懸念に対して、
**Stripe 1本でカバーできる**ことを確認した。

| 手段 | 対応 | 手数料 |
|---|---|---|
| クレジットカード（Visa / Mastercard / JCB / Amex / Diners / Discover） | ○ | **3.6%** |
| Apple Pay | ○ | 3.6% |
| Google Pay | ○ | 3.6% |
| **PayPay** | ○ | **3.98%** |
| コンビニ払い・銀行振込 | ○ | 銀行振込の返金のみ ¥250+税 |

- 初期費用・月額費用なし。決済が起きたときだけ課金される
- カード系は**返金手数料なし**。PayPay も全額／一部返金に対応
- PayPay は Checkout / Payment Element の両方で使える

**注意点2つ:**

1. **QR決済は PayPay のみ。** d払い・au PAY・楽天ペイは Stripe 非対応（2026-09時点）
2. **Apple Pay / Google Pay はアプリ不要**でブラウザから指紋・顔認証で払える。
   実は PayPay よりハードルが低い可能性があり、若年層対策として本命になりうる

出典: https://stripe.com/jp/payment-method/paypay / https://stripe.com/jp/pricing

## 今の作りとの相性は良い

- **受渡番号の仕組みが既にある**（`pickup_no` 採番・`/complete` 表示・厨房伝票）
- **テイクアウトの導線が既にある**（`orders.order_type='takeout'` / `/admin/pickup`）
- **決済画面は自前で作らない。** Stripe のホスト画面に飛ばせば、
  カード番号がこちらのサーバーを一切通らない（PCI の責任範囲が激減する）

## 設計の急所3つ。ここを外すと事故る

### ① 決済成功 **→** 注文作成 の順を絶対に守る

先に注文を作って後から決済にすると、**未払いの伝票が厨房で刷られる**。
必ず Stripe の webhook で決済完了を受けてから注文を作る。
現状は `orders` への insert がトリガーで `print_jobs` を作る
（`supabase/print_jobs.sql` の `enqueue_print_job`）ので、
**insert のタイミングが即・印刷のタイミング**である点に注意。

### ② レジとの二重請求

`/admin/register` は「これから会計する」前提で作られている。
事前決済の注文がそのまま流れると**店頭でもう一度請求してしまう**。
「支払済み」がレジ画面で一目で分かる必要がある。
`orders.status` の `paid` と、事前決済済みをどう区別するかを先に決めること。

### ③ 提供できなかったときの返金

売り切れ・機器トラブルは必ず起きる。**店頭で返金できる導線**が要る。
Stripe の管理画面を開かせるのは現場では無理。
返金権限は会計と同じく `register` / `manager` に限ること（RLS を緩めない）。

## 事業判断として（AI は判断しない）

飲食の原価率に対して 3.6〜3.98% は軽くない。
客単価1,500円なら1件あたり54〜60円、月200件で約1万2千円。
**混雑緩和とレジ対応削減で回収できるか**は洋輔と天真が決めること。

## 進め方

1. リオープンして通常営業を回す
2. **テイクアウトが実際どれくらい出るか見る**（月10件なら作る価値は薄い）
3. 数が出るなら実装。規模は 2〜3日程度と見ている

---

# 引き継ぎ（2026-09-04 時点）

次のセッションは**新機能から始める**。着手前にこの節を読むこと。

## 本番の状態

| | |
|---|---|
| URL（お客様） | `https://app.good-order.jp/yorkys-shukugawa/` |
| URL（管理） | `https://app.good-order.jp/yorkys-shukugawa/admin` |
| Vercel プロジェクト | `yorkys-orderly`（新規に作り直してはいない） |
| Supabase プロジェクト | **`oiropkuvaenebmlicrac`**（ダッシュボード上の名前は `good-order`） |
| basePath | `/yorkys-shukugawa`（`NEXT_PUBLIC_BASE_PATH`） |

データは**もう実データが入っている**（2026-09-04 時点で
卓10 / カテゴリ14 / メニュー27）。**もう白紙化してはいけない。**

## 厨房プリンタ（EPSON TM-m30III-H）— 設置・稼働済み

2026-08-27 に店舗で接続完了。**5秒間隔で稼働中。**

- 設定場所は **Web Config →「情報確認」タブ → 左メニュー下部の
  TM-Intelligent →「TM-i設定」→ サービス → サーバーダイレクトプリント**
  - **「印刷」タブでも「デバイス管理」でもない。**ここを探して30分溶かした
  - **管理者ログオンしないとこのメニューは出ない**（パスワードは `epson`）
  - 「クラウドサービス」は Epson Connect の別機能。触るとエラーが出るが無害
- ID / パスワード欄は**空でよい**（URL にトークンを埋める方式のため）
- 間隔は 5 秒、URLエンコード有効、サーバー認証無効、カット予約2秒
- プリンタの時計は UTC のままだが、**伝票の時刻はサーバー側が作るので正しく出る**
- 生存確認は `printer_status.last_seen_at`（数十秒以内なら生きている）

## 未処理のもの

1. **PR #50 が未マージ** — 終売メニューの扱いとエラー表示の修正。
   `fix/deletable-menu-and-readable-errors`
2. **使っていない Supabase プロジェクト `kugmyzvhwzphmuicdken` が生きている。**
   「新規プロジェクトで作る」と決めた時に作って SQL を24本流した後、
   本番は既存プロジェクトのままにしたので**宙に浮いている**。
   課金が続いている可能性がある。**中を確認してから**残すか消すか決めること
3. **スクリーンショットが撮れていない**（PR #48 / #50）。
   規約では PC 1400px / SP 390px の提示までが完了。
   **`npm run dev` が起動していなかった**ため撮れていない
   （ポート3000 で動いていたのは GOOD REVIEW という別プロジェクトだった）

## 次にやる新機能の候補は2つ。どちらも未着手

- **コラボレーション注文**（上の節）— 全員が卓の注文状況を共有して見る。B案推奨
- **テイクアウトの事前決済**（上の節）— Stripe。マクドナルド方式

どちらも「**リオープン後の実際の営業を見てから**」を前提に書いてある。
着手前に、その前提がまだ有効か天真に確認すること。

## 環境の注意

- `.env.local` は `oiropkuvaenebmlicrac` を指している。本番と同じ。
  **ローカルでの操作がそのまま本番データに効く**
- 画面の変更をしたら `npm run dev` を**天真に起動してもらってから**
  スクリーンショットを撮る（勝手に起動しない。規約 2 を参照）
- `vercel redeploy` は**使わない**。対象デプロイのコミットごと巻き戻る。
  詳細は「basePath を本番へ反映する手順」の節を参照
