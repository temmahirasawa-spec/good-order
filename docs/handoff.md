...
# 検討事項: コラボレーション注文（未着手・2026-08-27 発案）

天真の発案。**実装前に必ずこの節を読むこと。**

## 何を解こうとしているか

タブレット注文には無く、モバイルオーダーになって**生まれてしまった**問題がある。

> 卓に1台のタブレットなら全員で囲めた。スマホになった瞬間、
> **一人の端末が実質的な注文係になり、その人だけ会話から外れる。**

これは移行時に誰も設計しなかった副作用で、退化しているのに戻されていない。
現場で実際に起きること:

- 注文係の人がずっと画面を見ていて会話に入れない
- 「枝豆もう頼んだ？」の重複確認が毎回発生する
- 後から来た人が状況を把握できない
- 割り勘のとき、誰が何を頼んだか誰も覚えていない

## 「全員が同じ卓に注文できる」だけなら新規性は無い

居酒屋系のモバイルオーダーは卓単位で伝票がまとまる作りが普通なので、
**構造上すでに複数人が同じ卓に注文できている**（他社にもあると思われる）。

新しいのはそこではなく、**見える状態を全員で共有する**部分:

- 誰が何を頼んだか
- 今どれが作られていて、どれが届いたか
- あと何が来るのか

**他社調査は未実施。** 実装判断の前に調べること。

## 3案。B を推す

| 案 | 中身 | リスク |
|---|---|---|
| A | **カートを共有**。全員で1つのカートに入れ、誰かが確定 | 誰かが早く確定して事故る。調整UIが要る |
| B | **カートは個別、見える状態を共有**。注文は現行のまま各自、「この卓の注文」が全員にリアルタイムで見える | **低い。注文確定の処理に一切触らない** |
| C | B ＋ あとから「みんなのカート」を足す | B を作ってから判断 |

**B を推す理由:**

1. **注文確定の処理に触らずに作れる。** あそこは事故ると営業が止まる
   （`perf/order-confirm-latency` / 2026-08-26 の監査を参照）
2. 発案の価値（誰が何を・今どうなってる）は **B で全部出る**。
   A はその上の「一緒に選ぶ体験」で、無くても困らない
3. 使われ方を見てから A が要るか判断できる

## 材料はほぼ揃っている

- 卓のQR（`tables.short_code`）… ある
- 注文が卓に紐づく仕組み … ある
- リアルタイム更新（Supabase Realtime）… 既に使用中
- お客様側の注文履歴画面（`/history`）… ある

新規に要るのは実質**「注文時に、その人の呼び名も記録する」**だけ。
お客様側は未認証のままでよく、初回スキャン時にニックネームを1回聞いて
端末に保存する形で足りる（ログインは作らない）。

## 先に決めるべき危険な点が2つ

### 1. 隣の卓のQRを読まれたら見えてしまう

「注文できてしまう」問題は現行と同じだが、**共有ビューを足すと
「他人の注文が見える」プライバシー面が増える**。
会計が終わったら見えなくする（卓のセッションを区切る）設計が必須。

### 2. 割り勘への期待

誰が何を頼んだか見えると、お客様は当然「別々に払える」と思う。
レジ側が対応していないと**期待だけ作って裏切る**。
機能を出す前に、会計の運用とセットで決めること。

## いつやるか

1. 9月のリオープンを確実に迎える（優先）
2. **実際の営業を見る** — 本当に一人が注文係になっているか自分の目で確認する
3. それから B 案を作る

**2 を飛ばさないこと。** 仮説の筋は良いが、実店舗では想像と違うことが必ず出る。
