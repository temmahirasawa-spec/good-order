...
/* ══════════════════════════════════════════════════════════════
   移行先で書き換える設定。ここだけ直せば動く。
   ══════════════════════════════════════════════════════════════ */

/**
 * Figma ファイルのキー。
 * ファイルのURL https://www.figma.com/design/<ここ>/名前 の <ここ> の部分。
 * 環境変数 FIGMA_FILE_KEY を入れておけばそちらが優先される。
 */
const FILE_KEY = process.env.FIGMA_FILE_KEY || "{{FIGMA_FILE_KEY}}";

/** 検品しないページ（素材置き場・アーカイブなど）。無ければ空配列 [] */
const SKIP_PAGES = ["{{FIGMA_SKIP_PAGE}}"];
 ...
 */
const SCREEN_PAGES = ["{{FIGMA_SCREEN_PAGE}}"];

/** PC / SP の対を要求しないセクション（SP しか存在しない画面など）。無ければ空配列 [] */
const PAIR_EXEMPT_SECTIONS = ["{{FIGMA_PAIR_EXEMPT_SECTION}}"];
