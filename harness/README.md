...
1. **子孫に INSTANCE / COMPONENT が2個以上あるもの** — ボタンを並べている入れ物である。
   **「1個以上」にしないこと。** 生フレームのボタンはたいてい中に `Icon` インスタンスを1個持っており、
   1個で除外すると本物の生フレームボタンをまとめて見逃す（GOOD ORDER の実測で11件が消えた）
2. **高さが 120px を超えるもの** — ボタンにその高さはない。セクション帯や大きなカードである
3. **直接の子に、枠とほぼ同じ大きさ（幅または高さが90%以上）の INSTANCE / COMPONENT があるもの**
   — そのコンポーネントを包んでいるだけの容器である。
   **個数では容器と本物を区別できない**（`CTA Block` 342×77 の中に 342×52 のボタン1個 ＝ 容器、
   `Delete Button` 32×32 の中に 16×16 のアイコン1個 ＝ 本物）。違うのは大きさの比率。
...
セクション内のすべてのオートレイアウトフレームについて、余白と間隔がスケールに乗っているかを見る。

**余白・間隔は Spacing 変数（`space/*`）を使う。この数字で組むと美しくなるから。
中途半端な数字が1つ交じると全体の秩序が崩れていく。**

| 対象 | 内容 |
|---|---|
| ノード | セクション内のオートレイアウトフレーム。**インスタンス内部は対象外**（コンポーネント側で担保する） |
| プロパティ | `paddingTop` / `paddingRight` / `paddingBottom` / `paddingLeft` / `itemSpacing` / `counterAxisSpacing` |

| 状態 | 結果 |
|---|---|
| Spacing 変数にバインド済み | 合格 |
| 未バインドだが、値がスケール（2, 4, 8, 12, 16, 20, 24, 32, 40, 48, 64, 80, 96, 112, 128）または 0 | 合格 |
| 未バインドでスケール外 | **違反**（ソフトエラー。件数つきベースラインの対象） |

スケール値は実行時に Figma の `Spacing` コレクションから取得を試み、取れなければ
スクリプト内の表にフォールバックする。**どちらを使ったかは実行のたびに出力に出る。**

```
余白のスケール: ⚠ Figma から取得できないため、スクリプト内の表を使用（file_variables:read スコープが無いため）
```

`/v1/files/:key` の応答には変数の定義が含まれず、変数の値を返す `/variables/local` は
`file_variables:read` スコープが要る。そのため通常はフォールバックになる。
スコープ付きのトークンに替えれば、自動で本物のスケールを拾う。

> **⚠ 移植先で確認すること。**
> フォールバックのスケール値（2, 4, 8, 12, 16, 20, 24, 32, 40, 48, 64, 80, 96, 112, 128）は
> **GOOD ORDER / GOOD LOOP の既定値**である。
> 別のスケールを使っているプロジェクトでは、Figma の `Spacing` コレクションが
> 正しくスコープされていない限り**誤検出になる**。
> そのときは `SPACING_SCALE_FALLBACK` をそのプロジェクトの値に書き換えるか、
> `file_variables:read` スコープ付きのトークンを使うこと。
